
const PlainUserIcon = ({ size }) => {
  return (
    
    <svg id="fi_17571182" width={size} height={size} viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg" data-name="Layer 1" fill="rgba(255, 250, 250, 0.45)">
      <defs>
        <linearGradient id="gradient-full" x1="0%" y1="0%" x2="120%" y2="120%">
          <stop offset="0%" stop-color="rgba(99, 187, 216, 0.6)" />
          <stop offset="90%" stop-color="#ffffff00" />
        </linearGradient>
        <linearGradient id="gradient-half" x1="-50%" y1="-50%" x2="100%" y2="100%">
          <stop offset="0%" stop-color="#ffffff" />
          <stop offset="100%" stop-color="#ffffff00" />
        </linearGradient>
      </defs>
      <g><path d="m368.4 202.5c12.9-20.7 19.9-44.6 20-69.2.2-35.2-13.4-68.6-38.4-93.9-24.9-25.2-58-39.2-93.2-39.4-.2 0-.5 0-.7 0-35.3 0-68.4 13.7-93.4 38.5-25.2 25-39 58.4-39 93.9 0 24.9 6.9 49.1 20 70.1 16.8 26.9 42 46.4 71.1 55.9.7.2 1.4.4 2 .6 6.3 1.9 12.7 3.4 19.2 4.4.2 0 .4.1.6.1 1.3.2 2.6.4 4 .5.4 0 .8.1 1.2.1 1.1.1 2.3.2 3.4.3.5 0 1 .1 1.4.1 1.1.1 2.3.1 3.4.2.5 0 .9 0 1.4.1 1.6.1 3.1.1 4.7.1s3.1 0 4.7-.1c.5 0 .9 0 1.4-.1 1.1-.1 2.3-.1 3.4-.2.5 0 1-.1 1.5-.1 1.1-.1 2.3-.2 3.4-.3.4 0 .8-.1 1.2-.1 1.3-.2 2.6-.3 3.9-.5.2 0 .4-.1.7-.1 6.5-1 12.9-2.5 19.1-4.4.6-.2 1.2-.4 1.8-.5 29.1-9.5 54.3-29 71.2-56z"></path><path d="m8.8 417.2c-1.8 6.5.7 13.4 6 17.3 70.6 50.7 154 77.5 241.2 77.5s170.6-26.8 241.2-77.6c5.3-3.8 7.7-10.8 6-17.3-6.1-22.7-15.4-44.4-27.6-64.6-34.7-57.7-105-83.6-170.9-63.2-15.7 4.9-32.1 7.3-48.7 7.3s-33-2.5-48.7-7.4c-15.1-4.7-30.4-6.9-45.4-6.9-50.8 0-98.7 25.6-125.5 70.1-12.2 20.4-21.4 42.1-27.6 64.8z"></path></g>
      </svg>
  );
}

export default PlainUserIcon;
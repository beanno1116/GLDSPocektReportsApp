...
import { DAYS_OF_WEEK, dateDetails, previousMonth, nextMonth, daysInMonth, firstDayOfMonth, isDaySame, padValue, ABR_DAYS_OF_WEEK } from '../WEDateUtils';
...
const PickerModal = forwardRef(({ date, handleSelect, handleClose, show, top, left }, ref) => {
  ...
  const renderGridColumnHeadings = () => {
    return Object.values(ABR_DAYS_OF_WEEK).map((day, index) => {
      return (
  ...
  return (
    <div className={`${styles.picker_modal} ${show ? styles.showing : ""}`} onClick={handleClickOutside}>
      <div ref={ref} className={styles.date_picker_modal}>
